...
public class JogadorController {
    ...
    public ResponseEntity<List<Jogador>> getAllJogadores(@RequestParam(required = false) String nome){  //Coloca todos os jogadores em um vetor
        try {
            ...
            return new ResponseEntity<>(lj,HttpStatus.OK);       //Retorna uma entidade de resposta que informa que a ação foi realizada com sucesso
    ...
    @PutMapping("/jogadores/{cod_jogador}")  //Atualiza os atributos do jogador atravé de um ID
    public ResponseEntity<Jogador> updateJogador(@PathVariable("cod_jogador") int cod_jogador, @RequestBody Jogador j)
    {
        ...
        {
            Jogador jr = data.get();    //Pega os dados do jogador
